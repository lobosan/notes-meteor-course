...
if (Meteor.isClient) {
  describe('Editor', function () {
    ...
    it('should render not found message', function () {
      const wrapper = mount(<Editor browserHistory={browserHistory} meteorCall={meteorCall} selectedNoteId={notes[0]._id} />);
      expect(wrapper.find('p').text()).toBe('Note not found.');
    });

    it('should remove note', function () {
      const wrapper = mount(<Editor browserHistory={browserHistory} meteorCall={meteorCall} selectedNoteId={notes[0]._id} note={notes[0]} />);
